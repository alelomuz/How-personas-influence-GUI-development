...
Once the demographics and personas are defined, they can be used to improve the GUI development process in three complementary, not mutually exclusive categories: \textbf{Population-Level Adaptation (Static)}, \textbf{Individual-Level Adaptation (Dynamic)}, and \textbf{Process \& Validation Influence}.
...
The goal is to optimize the interface for the majority of the user population. This type of adaptation is especially powerful for characteristics that are relatively stable across users. It requires way less efforts compared to individual-level adaptation, since the GUI is designed once for a whole user group.

% When to use population-level adaptation instead of individual-level adaptation
There are a few scenarios where population-level adaptation is preferred over individual-level adaptation. Since such way of adaptation requires way less effort, in comparison to 
